...

{
	...
	private bool dieAnim =false;
	private string currentAnim;
	
	void animate(){
			if (movement.isMoving){
				if (jump.jumping) currentAnim = "soldierFalling";
				else if (Input.GetButton ("Shoot")) currentAnim = "soldierSprint";
				else if (Input.GetButton ("Crouch")) currentAnim = "soldierCrouchRun";
				else  currentAnim = "soldierRun";
			}
			else if(jump.jumping) currentAnim = "soldierFalling";
			else if (Input.GetButton ("Crouch")) currentAnim = "soldierCrouch";
			else if (dieAnim == true) currentAnim = "soldierDieFront";
			else currentAnim = "soldierIdle";
			animation.CrossFade(currentAnim);
	
		}
	
 	void OnTriggerEnter(Collider other)
	{
        if (other.tag == "Deathzone" && dieAnim == false)
		{
 ...
 // Require a character controller to be attached to the same game object
...

    {
        ...
        if (!jump.jumping) yield return new WaitForSeconds(this.animation["soldierDieFront"].length);
		Spawn();
		GameStats.takeLife();
		dieAnim = false;
...

    {
        ...
        //{
           ...
           if (Input.GetButton ("Shoot") && canControl)

                targetSpeed *= movement.runSpeed;

           else if (Input.GetButton ("Crouch") && canControl)
